...
// hadji hicham 23-07-2014 . inlinepatch for labels .
        $scope.inlinePatch = function (kind, edge, name, tag, value) {
            ...
            /*        $scope.save = function (contact) {
             ...
             Contact.insert($scope, contact, force);
...
app.controller('ContactNewCtrl', ['$scope', 'Auth', 'Contact', 'Account', 'Edge', 'Map', 'Linkedin',
    function ($scope, Auth, Contact, Account, Edge, Map, Linkedin) {
        ...
        $scope.save = function (contact, force) {
            ...
            console.log(test);
            if (typeof(contact.account) == 'object') {
                $scope.insertParams['account'] = contact.account.entityKey;
            } else if ($scope.searchAccountQuery) {
            ...
            if (!delayInsert) {
                if ($scope.profile_img.profile_img_id) {
                    $scope.insertParams['profile_img_id'] = $scope.profile_img.profile_img_id;
                    if ($scope.profile_img.profile_img_id) {
                        $scope.insertParams['profile_img_url'] = 'https://docs.google.com/uc?id=' + $scope.profile_img.profile_img_id;
                    } else {
                        if ($scope.profile_img.profile_img_url) {
                            $scope.insertParams['profile_img_url'] = $scope.profile_img.profile_img_url;
                        }
                    }
                }
                if ($scope.profile_img.profile_img_url) {
                ...
                Contact.create($scope, $scope.insertParams, force);
            }
        };
        ...
        $scope.saveCLinkedinUrl = function (shortProfile) {
            ...
            $scope.accountFromLinkedin = {
                ...
                'infonodes': [{
                    'kind': 'sociallinks',
                    'fields': [{'field': "url", 'value': shortProfile.url}]
                }, {'kind': 'websites', 'fields': [{'field': "url", 'value': shortProfile.website}]}],
        ...
        $scope.mergeContact = function (baseContact) {
            Contact.merge($scope, {base_id: baseContact.id, new_contact: $scope.insertParams});
        };
