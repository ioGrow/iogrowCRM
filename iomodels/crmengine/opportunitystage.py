...
class Opportunitystage (EndpointsModel):
    ...
    def list(cls,user_from_email,request):
        ...
        if request.order:
            ...
            if ascending:
                stages, next_curs, more =  cls.query().filter(cls.organization==user_from_email.organization).order(+attr).fetch_page(limit, start_cursor=curs)
            else:
                stages, next_curs, more = cls.query().filter(cls.organization==user_from_email.organization).order(-attr).fetch_page(limit, start_cursor=curs)
        else:
            stages, next_curs, more = cls.query().filter(cls.organization==user_from_email.organization).fetch_page(limit, start_cursor=curs)
        for stage in stages:
        ...
        for stage in stages :
            if (i<j):
                if stage.stage_number == i :
                    ...
                    stage.put()
                elif stage.stage_number > i and stage.stage_number <= j :
                    print "izaaaaaaaan"
                    stage.stage_number=stage.stage_number-1
                    stage.put()
            else:
                if stage.stage_number == i :
                    stage.stage_number=j
