...
def add_dashboard_tab(entity):
    ...
    if entity.name == "sales":
        ...
        if add_dashboard_tab:
            ...
            created_tab = Tab(name='Dashboard', label='Dashboard', url='/#/dashboard/', icon='dashboard',
...
def add_discovery_tab(entity):
    ...
    if entity.name == "sales":
        ...
        if add_discovery_tab:
            org_key = entity.organization
            created_tab = Tab(name='Discovery', label='Discovery', url='/#/discovers/', icon='twitter',
                              organization=org_key)
            tab_key = created_tab.put()
...
def create_new_tabs(entity):
    ...
    try:
        ...
        elif entity.label == "Settings":
            created_admin_tabs = []
            for tab in new_admin_tabs:
                created_admin_tab = Tab(name=tab['name'], label=tab['label'], url=tab['url'], icon=tab['icon'],
                                        organization=org_key)
...
def update_admin_tab_name(app):
    try:
        if app.label == 'Admin Console':
            app.label = 'Settings'
            yield op.db.Put(app)  # save the entity back to datastore
